...
public class Demo implements Observer{
	// create a java.util.Scanner object for use in all methods
		...
		private static CoinMachine machine;
		...
		public static void main(String[] args) {
			...
			machine = new CoinMachine( capacity );
			Demo demo = new Demo();
			machine.addObserver(demo);
			
			//run main UI
			CoinMachineUI UI1 = new CoinMachineUI(machine);
			machine.addObserver(UI1);
			UI1.setVisible(true);
			// run accepting coins UI
			AcceptingCoin UI2 = new AcceptingCoin(machine);
		...
		public void update(Observable o, Object arg) {
			String currency = "";
			if (machine.getCount() > 0) currency = machine.getCoins().get(0).getCurrency();
			System.out.printf("Machine contains value %d %s\n", machine.getBalance(), currency);	
		}
}
